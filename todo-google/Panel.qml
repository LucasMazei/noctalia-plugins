...
Item {
  ...
  Component.onCompleted: {
    if (pluginApi) {
      Logger.i("TodoGoogle", "Panel initialized");
      ...
      Rectangle {
        ...
        ColumnLayout {
          ...
          RowLayout {
            ...
            NText {
              text: pluginApi?.tr("panel.header.title") || "Todo List"
              font.pointSize: Style.fontSizeL
              font.weight: Font.Medium
              color: Color.mOnSurface
            }

            // Google Tasks sync status indicator
            Item {
              visible: pluginApi?.pluginSettings?.googleAccessToken
              Layout.preferredWidth: syncStatusRow.implicitWidth
              Layout.preferredHeight: syncStatusRow.implicitHeight
              ...
              RowLayout {
                ...
                NIcon {
                  icon: {
                    ...
                    if (status === "synced") return "cloud-done";
                    if (status === "syncing") return "cloud-sync";
                ...
                NIconButton {
                  ...
                  visible: pluginApi?.pluginSettings?.syncEnabled
                  onClicked: {
            ...
            NButton {
              ...
              text: pluginApi?.tr("panel.header.clear_completed_button") || "Clear Completed"
              icon: "trash"
          ...
          // Google Tasks sync error display
          Rectangle {
            Layout.fillWidth: true
            Layout.preferredHeight: syncErrorText.visible ? syncErrorText.implicitHeight + Style.marginM * 2 : 0
            visible: pluginApi?.pluginSettings?.syncError && pluginApi?.pluginSettings?.googleAccessToken
            ...
            RowLayout {
              ...
              NText {
                id: syncErrorText
                Layout.fillWidth: true
                text: "Sync Error: " + (pluginApi?.pluginSettings?.syncError || "Unknown error")
          ...
          ColumnLayout {
            ...
            RowLayout {
              ...
              NTextInput {
                id: newTodoInput
                placeholderText: pluginApi?.tr("panel.add_todo.placeholder") || "Add a new todo..."
                Layout.fillWidth: true
                ...
                function saveEdit() {
                  if (pluginApi && todoTextEdit.text.trim() !== "") {
                    ...
                    if (pluginApi.pluginSettings.syncEnabled && pluginApi.pluginSettings.googleAccessToken && pluginApi.pluginSettings.googleTaskListId) {
                      var mainInstance = pluginApi?.mainInstance;
                      if (mainInstance && mainInstance.syncTodoToGoogle) {
                        var todo = todos.find(function(t) { return t.id === modelData.id; });
                        if (todo) {
                          mainInstance.ensureValidToken(function(error, accessToken) {
                            if (!error) {
                              mainInstance.syncTodoToGoogle(todo, accessToken, pluginApi.pluginSettings.googleTaskListId);
                            }
                          });
                        }
                      }
                        ...
                        MouseArea {
                          ...
                          onClicked: {
                            ...
                            if (pluginApi) {
                              ...
                              if (indexToRemove !== -1) {
                                var todoToDelete = todos[indexToRemove];
                                ...
                                // The actual deletion will be handled by the sync process
                                if (todoToDelete && todoToDelete.googleTaskId && pluginApi.pluginSettings.syncEnabled && pluginApi.pluginSettings.googleAccessToken && pluginApi.pluginSettings.googleTaskListId) {
                                  var mainInstance = pluginApi?.mainInstance;
                                  if (mainInstance && mainInstance.syncWithGoogleTasks) {
                                    // Trigger a sync to handle the deletion in Google Tasks
                                    mainInstance.syncWithGoogleTasks();
                                  }
                                }
                              } else {
                                Logger.e("TodoGoogle", "Todo with ID " + modelData.id + " not found for deletion");
                              }
                            } else {
                              Logger.e("TodoGoogle", "pluginApi is null, cannot delete todo");
                            }
                          }
                        }
            ...
            Item {
              ...
              ColumnLayout {
                ...
                NText {
                  ...
                  text: {
                    var status = pluginApi?.pluginSettings?.syncStatus || "disconnected";
                    var lastSync = pluginApi?.pluginSettings?.lastSyncAt;
                    if (status === "synced" && lastSync) {
                      return "Last synced: " + new Date(lastSync).toLocaleString();
                    } else if (status === "syncing") {
                      return "Syncing...";
                    } else if (status === "error") {
                      return "Sync error: " + (pluginApi?.pluginSettings?.syncError || "Unknown");
                    }
                    return "";
                  }
                  color: Color.mOnSurfaceVariant
      ...
      if (pluginApi) {
        ...
        if (pluginApi.pluginSettings.syncEnabled && pluginApi.pluginSettings.googleAccessToken && pluginApi.pluginSettings.googleTaskListId) {
          ...
          if (mainInstance && mainInstance.syncTodoToGoogle) {
            mainInstance.ensureValidToken(function(error, accessToken) {
              if (!error) {
                mainInstance.syncTodoToGoogle(newTodo, accessToken, pluginApi.pluginSettings.googleTaskListId);
              }
            });
          }
        }
